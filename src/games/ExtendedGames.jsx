...
export function ReversiArena({game}) {
  ...
  return <GameFrame game={game} score={moves*8} step={turn==="B"?"YOUR TURN":"COACH"} onReset={reset}>
    <div className="reversi-layout"><section className="reversi-panel"><div className="board-score"><span>You <b>{counts.B}</b></span><i>REVERSI</i><span>Coach <b>{counts.W}</b></span></div><div className="reversi-board" role="grid" aria-label="Reversi board">{board.map((cell,index)=>{const isLegal=legal.includes(index)&&turn==="B";return <button key={index} role="gridcell" disabled={!isLegal} onClick={()=>place(index,"B")} className={isLegal?"legal":""} aria-label={"Row "+(Math.floor(index/8)+1)+", column "+(index%8+1)+(cell?(cell==="B"?", your disc":", coach disc"):isLegal?", legal move":", empty")}>{cell&&<i className={cell==="B"?"black":"white"}/>}</button>;})}</div><p aria-live="polite">{message}</p></section><aside className="lesson-panel"><span className="panel-label">STRATEGY LENS</span><h2>Win the final position.</h2><p>Every move must trap one or more opposing discs in a straight line. The player with more discs when neither side can move wins.</p><div className="tip"><Lightbulb/>Corners can never be flipped. Avoid giving them away.</div></aside></div>
  </GameFrame>;
}
...
export function NonogramStudio({game}){
  ...
  return <GameFrame game={game} score={cells.filter(v=>v===1).length*5} step={"PUZZLE "+(round%nonograms.length+1)} onReset={reset}><section className="nonogram-stage"><div><span className="panel-label">PIXEL LOGIC</span><h1>Read every line.</h1><p aria-live="polite">{message}</p></div><div className="nonogram-wrap"><div/><div className="column-clues" aria-label="Column clues">{columns.map((line,i)=><span key={i}>{clue(line).join(" ")}</span>)}</div><div className="row-clues" aria-label="Row clues">{puzzle.grid.map((line,i)=><span key={i}>{clue(line).join(" ")}</span>)}</div><div className="nonogram-board" role="grid" aria-label="Nonogram puzzle grid">{cells.map((value,index)=><button key={index} role="gridcell" className={value===1?"filled":value===2?"crossed":""} onClick={()=>setCells(list=>list.map((cell,i)=>i===index?(cell+1)%3:cell))} aria-label={"Row "+(Math.floor(index/5)+1)+", column "+(index%5+1)+(value===1?", filled":value===2?", marked empty":", unknown")}>{value===2?"×":""}</button>)}</div></div><div className="puzzle-actions"><p><Lightbulb/>Tap once to fill, twice to mark empty.</p><button className="cta" onClick={inspect}><Check/>Check pattern</button></div></section></GameFrame>;
}
...
export function WordArchitect({game}){
  ...
  return <GameFrame game={game} score={score} step={seconds+"s"} onReset={reset} onStart={()=>setActive(true)}><section className="word-stage"><span className="panel-label">VOCABULARY CONSTRUCTION</span><h1>Build beyond the obvious.</h1><div className="letter-rack" aria-label={"Available letters: "+data.letters.split("").join(", ")}>{data.letters.split("").map((letter,index)=><span key={index}>{letter}<small>{index+1}</small></span>)}</div><form onSubmit={event=>{event.preventDefault();submit();}}><input value={entry} onChange={event=>setEntry(event.target.value.replace(/[^a-z]/gi,""))} placeholder="Type a word…" autoComplete="off" aria-label="Word"/><button type="submit" className="cta">Add word</button></form><p className="word-message" aria-live="polite">{message}</p><div className="found-words" aria-live="polite">{found.length?found.map(word=><span key={word}>{word}<b>+{word.length*word.length*3}</b></span>):<i>Your valid words will appear here.</i>}</div><button type="button" className="text-action" onClick={()=>setSeconds(0)}>Finish round</button></section></GameFrame>;
}
...
export function KakuroVault({game}){
  ...
  return <GameFrame game={game} score={values.filter(Boolean).length*8} step="SUM GRID" onReset={reset}><section className="kakuro-stage"><span className="panel-label">INTERSECTING SUMS</span><h1>Every crossing matters.</h1><p aria-live="polite">{message}</p><div className="kakuro-grid" role="grid" aria-label="Kakuro grid"><div className="clue dark"/>{columnSums.map(sum=><div className="clue" key={sum}>↓ {sum}</div>)}{rows.map((row,r)=><div className="kakuro-row" key={r}><div className="clue">→ {rowSums[r]}</div>{row.map((value,c)=>{const index=r*3+c;return <button key={index} role="gridcell" aria-selected={selected===index} aria-label={"Row "+(r+1)+", column "+(c+1)+(value?", "+value:", empty")} className={selected===index?"selected":""} onClick={()=>setSelected(index)}>{value||""}</button>;})}</div>)}</div><div className="number-pad" aria-label="Kakuro number pad">{[1,2,3,4,5,6,7,8,9].map(number=><button key={number} aria-label={"Enter "+number} onClick={()=>setValues(list=>list.map((v,i)=>i===selected?number:v))}>{number}</button>)}</div><div className="puzzle-actions"><p><Lightbulb/>Digits may not repeat within a run.</p><button className="cta" onClick={inspect}><Check/>Check sums</button></div></section></GameFrame>;
}
